...
for player in ivalues(GAMESTATE:GetHumanPlayers()) do
	...
	local Combo = STATSMAN:GetAccumPlayedStageStats():GetPlayerStageStats(player):GetCurrentCombo();
	...
	t[#t+1] = Def.ActorFrame{
		...
		LoadFont("_stars")..{
			InitCommand=function(self)
				local percent = STATSMAN:GetAccumPlayedStageStats():GetPlayerStageStats(player):GetPercentDancePoints()*100;
		...
		Def.GraphDisplay{
			...
			BeginCommand=function(self)
				...
				local playerStageStats = STATSMAN:GetAccumPlayedStageStats():GetPlayerStageStats(player)
