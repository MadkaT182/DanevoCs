local function CreditsText()
	local text = LoadFont("Common normal") .. {
		InitCommand=cmd(x,SCREEN_CENTER_X-27;y,SCREEN_BOTTOM-7;zoom,.5;playcommand,"Refresh");
...
local function CoinsText()
	local text = LoadFont("Common normal") .. {
		InitCommand=cmd(x,SCREEN_CENTER_X+74;y,SCREEN_BOTTOM-7;zoom,.5;playcommand,"Refresh");
		RefreshCommand=function(self)
			local coins=GAMESTATE:GetCoins()
			local coinsPerCredit=PREFSMAN:GetPreference('CoinsPerCredit')
			local remainder=math.mod(coins,coinsPerCredit)
